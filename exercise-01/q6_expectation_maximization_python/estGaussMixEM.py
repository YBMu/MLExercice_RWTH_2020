...
def estGaussMixEM(data, K, n_iters, epsilon):
    ...
    # EM
    for itrs in range(n_iters):

        prevLogLike, gamma = EStep(means, covariances, weights, data)
        weights, means, covariances, currLoglike = MStep(gamma, data)
        ...
        if abs(prevLogLike - currLoglike) < 1:
            break
